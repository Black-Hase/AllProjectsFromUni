...
def union(A,B):
    ...
    if len(components[A]) < len(components[B]):
        for i in components[A]:
            leader_dict[i]=B
            components[B].append(i)
    else if len(components[A]) > len(components[B]):
        for j in components[B]:
            leader_dict[j]=A
            components[A].append(j)
    pass
...
if __name__ == '__main__':
    ...
    for (u,v) in sorted_edges:
        u_leader=find(u)
        v_leader=find(v)
        if v_leader!=u_leader:
            union(v_leader,u_leader)
            kruskals_selected_edges.append((u,v))

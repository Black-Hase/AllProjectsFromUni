...
int byteSwap(int x, int n, int m) //done
{
    ...
    */
    ...
    int mTHByte = (mask2 & x);
    mask2 =(mask2&x);
    int joinMask = (mask | mask2);
    joinMask = ~joinMask;
    int joinMask2 =(joinMask & x);
    nTHByte = (nTHByte >> nNumShift);
    nTHByte = (nTHByte & 0xff);
    nTHByte =(nTHByte << mNumShift);
    mTHByte = (mTHByte>> mNumShift);
    mTHByte =(mTHByte & 0xff);
    mTHByte = (mTHByte << nNumShift);
    int result = (joinMask2 | mTHByte);
    result = (result | nTHByte);
    return result;
...
int satAdd(int x, int y) //worked with Spencer
{
    ...
    */
    ...
    test = test >> 31;
    int test2 = test &  0x1;
    overflow = (( valueY ^ test2) & (valueX ^ test2));
    
    int notZeroes = ~0x00;
    int sumSign = !(test2);
    mask = (!overflow + notZeroes);
    int mask2 = (!sumSign + notZeroes);
    T = ((~mask2) & V) | ((mask2) & a);

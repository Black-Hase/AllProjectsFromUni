...
def decrypt(ciphertext, key):
	...
	for i in range(len(cipher_ascii)):
		plain_ascii.append(((cipher_ascii[i]-key_ascii[i % len(key)]) % 26) +65) ## i Mod len key to keep it in the range of our key length then mod 26 to find the appropriat letter in the alphabet, then plus 65 to ket it to be the correct value in the ascii table. 
...
def main(cipher):
   # ciphertext_unfiltered = sys.argv[1]
